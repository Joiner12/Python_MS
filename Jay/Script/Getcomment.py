...
headers = {
    'User-Agent':
    random.choice(user_agent),  # 随机选取头部代理
    'Connection':
    "keep-alive",
    'Host':
    "music.163.com",
...
def get_ip_list(url, headers):
    ...
    for i in range(1, len(ips)):
        ...
        ip_list1.append('http://' + tds[0].text + ':' +
                        tds[1].text)  #kuaidaili
    return ip_list1
...
def main():
    ...
    # 请求r1，获取随机代理IP
    r1 = requests.get(
        'http://47.100.21.174:8899/api/v1/proxies?limit=60').json()
        ...
        for i in range(10000):
            ...
            if json:
                ...
                for comment in comments:
                    ...
                    print(user_name, user_id, com, comment_id, praise,
                          structed_date)
                    f.write(user_name + ',' + user_id + ',' + com + ',' +
                            comment_id + ',' + praise + ',' + structed_date +
                            '\n')
                offset += 20
